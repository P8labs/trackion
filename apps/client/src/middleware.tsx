...
export function RouteMiddleware({ children }: { children: React.ReactNode }) {
  ...
  // Identify if the current path is meant for the web only
  const isWebOnlyRoute = webOnlyRoutePrefixes.some((path) => {
    if (path === "/") return location.pathname === "/"; // Exact match so it doesn't trigger on every route
    if (path === "*") return false; // Ignore wildcard to avoid breaking 404 behavior
    return location.pathname.startsWith(path);
  });

  // --------------------------------------------------------
  // Mobile / Desktop App Web-Only Route Blocking
  // --------------------------------------------------------
  if (!IsWeb() && isWebOnlyRoute) {
