...
func (h *handler) GetRecentEvents(w http.ResponseWriter, r *http.Request) {
	...
	paginatedEvents, err := h.service.GetRecentEvents(r.Context(), projectId, page, pageSize)
	if err != nil {
		log.Println(err)
		res.Error(w, err.Error(), 500)
		return
...
func (h *handler) GetRealtimeEvents(w http.ResponseWriter, r *http.Request) {
	projectId := chi.URLParam(r, "id")
	...
	events, err := h.service.GetRealtimeEvents(r.Context(), projectId, int32(limit))
	if err != nil {
		log.Println(err)
		res.Error(w, err.Error(), 500)
		return
