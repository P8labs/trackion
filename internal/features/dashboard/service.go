...
func (s *Service) GetAreaChartData(ctx context.Context, projectId string, timeRange string, eventFilter string) ([]AreaChartDataPoint, error) {
	projectUUID := uuid.MustParse(projectId)
	...
	for i, r := range rows {
		result[i] = AreaChartDataPoint{
			Period:  r.Period.Format(time.RFC3339),
			Desktop: r.Desktop,
			Mobile:  r.Mobile,
		}
	}
...
func (s *Service) GetOnlineUsers(ctx context.Context, projectId string) (int64, error) {
	projectUUID := uuid.MustParse(projectId)
...
func (s *Service) GetCountryMapData(ctx context.Context, projectId string) (*CountryMapData, error) {
...
func (s *Service) GetRecentEvents(ctx context.Context, projectId string, page, pageSize int32) (*PaginatedEventsResponse, error) {
	if page < 1 {
		page = 1
	}
...
func (s *Service) GetRealtimeEvents(ctx context.Context, projectId string, limit int32) ([]RealtimeEventData, error) {
	projectUUID := uuid.MustParse(projectId)
	...
	result := make([]RealtimeEventData, len(rows))

	for i, r := range rows {
		result[i] = RealtimeEventData{
			ID:        r.ID,
			EventName: r.EventName,
			EventType: deref(r.EventType),
			UserID:    deref(r.UserID),
...
func (s *Service) GetDeviceAnalytics(ctx context.Context, projectId string) (*DeviceAnalyticsData, error) {
	projectUUID := uuid.MustParse(projectId)
...
func (s *Service) GetTrafficSources(ctx context.Context, projectId string) (*TrafficSourcesData, error) {
	projectUUID := uuid.MustParse(projectId)
...
func (s *Service) GetTopPages(ctx context.Context, projectId string) ([]TopPage, error) {
	projectUUID := uuid.MustParse(projectId)
...
func (s *Service) GetCountryData(ctx context.Context, projectId string) ([]BreakdownItem, error) {
	projectUUID := uuid.MustParse(projectId)
