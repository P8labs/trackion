...
export function BreakdownSection({ projectId }: BreakdownSectionProps) {
  ...
  const browserChartConfig = data.browsers.reduce((config, browser, index) => {
    // Use the actual name as the key instead of transforming it
    config[browser.name] = {
      label: browser.name,
      color: browser.color || `var(--chart-${(index % 5) + 1})`,
    };
    return config;
  }, {} as ChartConfig);
                  ...
                  <div className="h-2 rounded-full bg-muted overflow-hidden">
                    <div
                      ...
                      style={{
                        ...
                        backgroundColor: device.color || "var(--chart-1)",
                      }}
                    />
                  </div>
                </div>
              ...
              <PieChart>
                ...
                >
                  {data.browsers.map((browser, index) => (
                    <Cell
                      key={browser.name}
                      fill={browser.color || `var(--chart-${(index % 5) + 1})`}
                    />
                  ))}
                </Pie>
                  ...
                  <div className="h-2 rounded-full bg-muted overflow-hidden">
                    <div
                      ...
                      style={{
                        ...
                        backgroundColor: referrer.color || "var(--chart-1)",
                      }}
                    />
                  </div>
                </div>
